<?php
	$linkMan = $linkFilter = "index.php?com=news&act=man_sub&type=".$type;
	$linkAdd = "index.php?com=news&act=add_sub&type=".$type;
    $linkEdit = "index.php?com=news&act=edit_sub&type=".$type;
    $linkDelete = "index.php?com=news&act=delete_sub&type=".$type;
?>
<!-- Content Header -->
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) {
                            ...
                            <tr>
                                ...
                                <td class="align-middle">
                                    <input type="number" class="form-control form-control-mini m-auto update-numb" min="0" value="<?=$items[$i]['numb']?>" data-id="<?=$items[$i]['id']?>" data-table="news_sub">
                                </td>
                                <?php if(isset($config['news'][$type]['show_images_sub']) && $config['news'][$type]['show_images_sub'] == true) { ?>
                                    <td class="align-middle">
                                    	<a href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['namevi']?>">
                                            <?=$func->getImage(['class' => 'rounded img-preview', 'sizes' => $config['news'][$type]['thumb_sub'], 'upload' => UPLOAD_NEWS_L, 'image' => $items[$i]['photo'], 'alt' => $items[$i]['namevi']])?>
                                        </a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
                                ...
                                <?php if(isset($config['news'][$type]['check_sub'])) { foreach($config['news'][$type]['check_sub'] as $key => $value) { ?>
                                    <td class="align-middle text-center">
                                        <div class="custom-control custom-checkbox my-checkbox">
                                            <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" data-table="news_sub" data-id="<?=$items[$i]['id']?>" data-attr="<?=$key?>" <?=(in_array($key, $status_array)) ? 'checked' : ''?>>
