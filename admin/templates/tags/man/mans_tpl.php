<?php
    ...
    $linkAdd = "index.php?com=tags&act=add&type=".$type;
    $linkEdit = "index.php?com=tags&act=edit&type=".$type;
    $linkDelete = "index.php?com=tags&act=delete&type=".$type;
?>
<!-- Content Header -->
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <input type="number" class="form-control form-control-mini m-auto update-numb" min="0" value="<?=$items[$i]['numb']?>" data-id="<?=$items[$i]['id']?>" data-table="tags">
                                </td>
                                <?php if(isset($config['tags'][$type]['show_images']) && $config['tags'][$type]['show_images'] == true) { ?>
	                                <td class="align-middle text-center">
	                                    <a href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['namevi']?>">
                                            <?=$func->getImage(['class' => 'rounded img-preview', 'sizes' => $config['tags'][$type]['thumb'], 'upload' => UPLOAD_TAGS_L, 'image' => $items[$i]['photo'], 'alt' => $items[$i]['namevi']])?>
                                        </a>
	                                </td>
	                            <?php } ?>
                                <td class="align-middle">
                                ...
                                <?php if(isset($config['tags'][$type]['check'])) { foreach($config['tags'][$type]['check'] as $key => $value) { ?>
                                    <td class="align-middle text-center">
                                        <div class="custom-control custom-checkbox my-checkbox">
                                            <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" data-table="tags" data-id="<?=$items[$i]['id']?>" data-attr="<?=$key?>" <?=(in_array($key, $status_array)) ? 'checked' : ''?>>
