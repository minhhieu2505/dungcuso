...
if (empty($id_order)) $func->transfer("Dữ liệu không có thực", "index.php?source=order&act=man", false);
$order_detail = $d->rawQueryOne("select * from `order` where id = ? limit 0,1", array($id_order));
...
$PHPExcel->getProperties()->setCreator($setting['name']);
$PHPExcel->getProperties()->setLastModifiedBy($setting['name']);
...
$PHPExcel->setActiveSheetIndex(0)->setCellValue('A1', $setting['name']);
...
$PHPExcel->setActiveSheetIndex(0)->setCellValue('D8', 'Tình trạng: ' . $arr_status[$order_status-1]);
...
/* Lấy và Xuất dữ liệu */
$position = 11;
$items = $d->rawQuery("select * from order_detail where id_order = ?", array($id_order));

for ($i = 0; $i < count($items); $i++) {
    $itemPro = $d->rawQueryOne("select * from product where id = ?", array(@$items[$i]['id_product']));
    $regular_price = $func->formatMoney($itemPro['regular_price']);
    $sale_price = $func->formatMoney($itemPro['sale_price']);
    $sum_price = 0;
    if (isset($itemPro['sale_price']) && $itemPro['sale_price'] > 0) $sum_price = $func->formatMoney($itemPro['sale_price'] * @$items[$i]['quantity']);
    else $sum_price = $func->formatMoney($itemPro['regular_price'] * @$items[$i]['id_product']);

    $PHPExcel->setActiveSheetIndex(0)
        ->setCellValue('A' . $position, $i + 1)
        ->setCellValue('B' . $position, $itemPro['name'])
