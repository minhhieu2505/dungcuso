...
<?php if (!empty($rowDetail['content'])) { ?>
    <div class="content-main content-ck w-clear"><?= htmlspecialchars_decode($rowDetail['content']) ?></div>
<?php } else { ?>
    ...
    <div class="share othernews mb-3">
        ...
        <ul class="list-news-other">
            <?php foreach ($news as $k => $v) { ?>
                <li><a class="text-decoration-none" href="<?= $v['slugvi'] ?>" title="<?= $v['name'] ?>"><?= $v['name'] ?> - <?= date("d/m/Y", $v['date_created']) ?></a></li>
            <?php } ?>
        </ul>
